...
def centerColumns(X):
    """centers a data matrix X by column

    returns X - mean(X), where the mean is taken over the columns of X"""

    mean = numpy.mean(X, 0)
    return X - numpy.resize(mean, (len(X), len(mean)))
...
def standardizeColumns(X):
    """
    Standardize a data matrix X by column
    
    returns (X - mean(X)) / std(X), where the mean and standard deviation
    are computed over the columns of X"""

    mean = numpy.mean(X, 0)
    std = numpy.std(X, 0)
    n = len(X)
    return (X - numpy.resize(mean, (n, len(mean)))) / numpy.resize(std, (n, len(std)))
    ...
    **caveat:**
    ...
    def train(self, data, **args):
        ...
        if self.rescale:
            ...
            # values that are equal to 0 are replaced with a value of 1
            eps = 1e-5
