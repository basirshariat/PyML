...
class ClassificationFunctions(object):
    ...
    def formatConfusionMatrix(self):
        ...
        rep = []
        columnWidth = 1 + max(4, max([len(label) for label in self.classLabels]))
        columnWidth = max(columnWidth,
                          int(math.ceil(math.log10(numpy.max(self.confusionMatrix))) + 1))

        rep.append('Confusion Matrix:')
        rep.append(' ' * columnWidth + ' predicted labels:')
...
class ClassificationResultsContainer(ResultsContainer, ClassificationFunctions):
    ...
    def computeStats(self, **args):
        ...
        self.confusionMatrix = superConfmat(self.givenY, self.Y, self.numClasses)
...
class RegressionResultsContainer(ResultsContainer):
    ...
    def computeStats(self):
        ...
        self.mad = numpy.average([abs(self.givenY[i] - self.Y[i]) for i in range(len(self.Y))])


class RegressionResults(Results):
    attributeAction = {'rmse': 'average', 'mad': 'average'}
    ...
    def getMAD(self, fold=None):

        return self.get('mad', fold)
